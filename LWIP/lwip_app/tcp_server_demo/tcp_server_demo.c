...
//mydebug_12.27
extern u8 recbuff[TCP_SERVER_RX_BUFSIZE];
//TCP服务器发送数据内容
...
//mydebug_12.27
u16 glength=0;
u8 send_number=0;    //记录需要发送多少次，64B为一次
extern USBH_HOST  USB_Host;
extern USB_OTG_CORE_HANDLE  USB_OTG_Core;

static const u8 *fxpath="0:/12_27.txt";	 
...
void tcp_server_test(void)
{
	...
	//打开文件记录收到的数据
	res=f_open(&fftemp,(const TCHAR*)fxpath,FA_READ|FA_WRITE|FA_CREATE_NEW);
	if(res)
	{
	...
	while(res==0)
	{
		...
		if(tcp_server_flag&1<<6)//是否收到数据?
		{
			...
			//md_send(tcp_server_recvbuf,glength);
			memcpy(recbuff,tcp_server_recvbuf,glength);
			
			res=f_write(&fftemp,tcp_server_recvbuf,glength,(UINT*)&bread);
			if(res)
			{
